    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-950 flex items-center justify-center p-6">
      <div className="grid w-full max-w-5xl rounded-2xl overflow-hidden shadow-xl grid-cols-1 md:grid-cols-2 bg-white dark:bg-gray-900">
        ...
        <div className="p-8 md:p-10">
          <h1 className="text-3xl font-semibold text-gray-800 dark:text-white/90 mb-2">Đăng ký</h1>
          <p className="text-gray-500 dark:text-gray-400 mb-6">Tạo tài khoản người dùng (không có quyền admin).</p>
          <form onSubmit={onSubmit} className="space-y-4">
            <div>
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Họ tên</label>
              <input className="w-full rounded-lg border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 focus:ring-brand-500 focus:border-brand-500" placeholder="Họ tên" value={name} onChange={(e)=>setName(e.target.value)} />
            </div>
            <div>
            ...
            <button type="submit" disabled={isPending} className="w-full rounded-lg bg-brand-500 hover:bg-brand-600 text-white py-3 shadow-theme-xs">{isPending? 'Đang xử lý...':'Tạo tài khoản'}</button>
          </form>
        </div>
      </div>
    </div>
